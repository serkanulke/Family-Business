...
const WORKER_SELECTION_SCENE := preload(
	"res://UI/Business/WorkerSelection/WorkerSelectionSheet.tscn"
)
...
func _open_worker_type_sheet() -> void:
	...
	sheet.source_selected.connect(
		_on_source_selected
	)

	sheet.cancelled.connect(
		_on_cancelled
	)


func _open_worker_selection_sheet(
	source_type: String
) -> void:
	_clear_active_sheet()

	var sheet := WORKER_SELECTION_SCENE.instantiate()
	active_sheet = sheet
	...
	sheet.candidate_selected.connect(
...
func _on_selection_cancelled() -> void:
	# Closing the candidate list returns to worker type selection.
	# No employment data is changed.
	_open_worker_type_sheet()
