...
This document describes the repository state inspected through 2026-08-28 at branch `main`, commit `86a220c` (`Ui elements`), plus the current working-tree Item List / Shop, Map, and Character portrait/genetics integrations. It is based on `project.godot`, the GDScript files, JSON resources, scenes, UI files, and test scenes present in the working tree. It does not reproduce or reinterpret the canonical GDD.
...
- `Scenes/Main/Main.tscn` owns a persistent `FamilyTreeScreen`, lazily instantiates `UI/Map.tscn`, and instances one `MainHUD`. Screen changes explicitly toggle content visibility/processing and the two independent Camera2D nodes, then update the shared navigation active state without rebuilding either screen.
...
- `UI/Map.tscn` currently contains manually authored TileMapLayer ground/road/environment content and building sprites under the organized `MapWorld` layers. This runtime repair does not alter their placement, TileSets, or artwork. `Interactive` and `Decorative` are Node2D containers so MapScreen visibility propagates to every building canvas item.
...
- `MapProperty`, `MapPropertyTag`, and `MapCoordinateHelper` remain reusable but are not instantiated by the currently authored Map scene. When a reusable family-business property has a business type, its building image resolves through `BusinessManager.get_business_map_visual_path()`. Property/business interaction wiring remains deferred while the project owner manually authors the city.
