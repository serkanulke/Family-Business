...
static func build(
	...
	var business_type: Dictionary = (
		BusinessManager.get_business_type_by_id(
			business_type_id
		)
	)
	...
	var breakdown: Dictionary = (
		BusinessManager.get_business_monthly_breakdown(
			business_instance_id
		)
	)
	...
	var next_upgrade := _build_next_upgrade(
		business_type_id,
		level
	)
...
static func _build_slots(
	...
	for slot_value in slots_value:
		...
		var row: Dictionary = {
			...
			"required_stats": _format_required_stats(
				slot_definition
			),
			"assigned_character_id": slot.get(
				"assigned_character_id",
				null
			),
			"assigned_npc_id": slot.get(
				"assigned_npc_id",
				null
			),
			"potential_income": int(
				slot_definition.get(
					"base_gross_contribution",
					0
				)
			),
			"income": BusinessManager.get_business_slot_gross(
				business_instance_id,
				slot_id
			)
		}

		_add_worker_presentation(
			row,
			slot,
			slot_definition
		)
...
static func _format_required_stats(
	slot_definition: Dictionary
) -> Array:
	var result: Array = []

	var stats_value = slot_definition.get(
		"required_stats",
		{}
	)
...
static func _add_worker_presentation(
	...
	if character_id_value != null:
		var character: Dictionary = (
			CareerManager.get_character_by_id(
				int(character_id_value)
			)
		)
		...
		row["worker_name"] = _full_name(
			character
		)

		row["portrait_path"] = (
			CharacterManager.get_avatar_path(
				character
			)
		)
	...
	var npc_manager: Node = scene_tree.root.get_node_or_null(
		"NPCManager"
	)
...
static func _build_next_upgrade(
	business_type_id: String,
	current_level: int
) -> Dictionary:
	var business_type: Dictionary = (
	...
	var current_slots_value = current_definition.get(
		"slot_ids",
		[]
	)
	...
	var next_slots_value = next_definition.get(
		"slot_ids",
		[]
	)
	...
	var current_expense := (
		BusinessManager.get_level_fixed_monthly_expense(
			business_type_id,
			current_level
		)
	)

	var next_expense := int(
		next_definition.get(
	...
	if unlocked_slot_names.size() == 1:
		slot_text = "+ New %s Slot" % (
			unlocked_slot_names[0]
		)
	elif unlocked_slot_names.size() > 1:
		slot_text = "+ %d New Staff Slots" % (
			unlocked_slot_names.size()
		)
	...
	return {
		...
		"monthly_expense": maxi(
			next_expense - current_expense,
			0
		),
		"cost": int(
			next_definition.get(
