...
func _apply_plan(plan: Dictionary, source_instance_id: String, created_items: Dictionary) -> Dictionary:
	...
	match effect_type:
		...
		"accept_job_offer":
			...
			var character := CharacterManager.get_character_by_id(character_id)
			var job_id := int(character.get("job_id", 0))
			var company_id := String(character.get("company_id", ""))
		...
		"job_remove":
			var character := CharacterManager.get_character_by_id(character_id)
			var previous_job_id := int(character.get("job_id", 0))
			var previous_company_id := String(character.get("company_id", ""))
